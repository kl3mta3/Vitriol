...
# Transmute generates. They run on the END USER's machine when they
...
# 16-byte sentinel prepended to the inner runtime BEFORE it is
# AES-CTR-encrypted. The loader stub checks for this exact prefix after
# decryption — match means the password was correct (the runtime decrypted
# to valid Python source with the expected magic). Mismatch means wrong
# password (decryption produced garbage), and the stub increments the
# counter accordingly.
_PY_RUNTIME_SENTINEL = b"<<TRANSMUTE-RT>>"
...
# Same PBKDF2 parameters as Transmute's Stone-v3 envelope crypto
...
# "Transmute" or "PyAttempts" so a casual regedit / file-system snoop
# can't find it by searching for the obvious term. The numeric folder
# name looks like a session/cache identifier — blends in with the dozens
# of similar-looking entries under HKCU\Software on a typical install.
_REGISTRY_KEY = r"Software\_872676883"
_POSIX_STORE_BASENAME = ".872676883"          # hidden file in user home
_MAX_ATTEMPTS = 5
...
#
#   1. Storage location is NOT named after Transmute. Windows uses
...
def _attempts_storage_path():
    ...
    Transmute — looks like a generic cache file."""
    import pathlib
...
# 16-byte sentinel that prefixes the encrypted inner runtime BEFORE
# encryption. After decryption the stub checks for this exact prefix —
# match = correct password; mismatch = wrong password (counter gets
# incremented). The constant is visible in the stub but the encrypted
# inner runtime itself is opaque, so an attacker can't simply read the
# rebuild logic from the .py file.
_RUNTIME_SENTINEL = b"<<TRANSMUTE-RT>>"
...
def main():
    ...
    # Decrypt the inner runtime body. Wrong password → garbage bytes,
    # the sentinel check below catches it. Right password → valid
    # Python source we can exec.
    ...
    if not rt_src.startswith(_RUNTIME_SENTINEL):
        # Wrong password (or tampered _runtime blob). Counter increments;
    ...
    # Sentinel matched — password is correct. Hand control to the
    # inner runtime. It uses the same pw_key to decrypt _data, runs
    # the animation, writes the file, clears the counter, self-deletes.
    inner_globals = dict(globals())
    inner_globals["_pw_key"] = pw_key
    inner_globals["_file_id"] = file_id
    exec(compile(rt_src[len(_RUNTIME_SENTINEL):], "<runtime>", "exec"),
         inner_globals)
...
def _py_embed_to_file_plain(src_path: Path, src_ext: str, dst: Path,
    ...
    with open(src_path, "rb") as f, open(dst, "w", encoding="utf-8", newline="") as out:
...
def _py_embed_to_file_encrypted(src_path: Path, src_ext: str, dst: Path,
    ...
    # against. Hash bytes are exactly piece1+piece2 — the file on
    # disk will have `_runtime = (...)\n)\n` between them, which the
    # runtime explicitly skips when re-hashing at execution time.
    prefix_bytes = piece1.encode("utf-8") + piece2.encode("utf-8")
    ...
    inner_plaintext = (_PY_RUNTIME_SENTINEL
                        + inner_stripped.encode("utf-8"))
    ...
    # Write the final file in BINARY mode so newlines are LF on every
    # platform (no Windows CRLF translation). The runtime hashes the
    # file as raw bytes; consistent newlines are required for the
    # embed-time hash and runtime-time hash to agree.
    with open(dst, "wb") as out:
        out.write(piece1.encode("utf-8"))
        out.write(runtime_block.encode("utf-8"))
        out.write(piece2.encode("utf-8"))


def _py_extract_to_file(src: Path, dst_path: Path,
                         cancel: Optional["CancellationToken"] = None,
                         password: bytes = b"") -> str:
    """If `src` is a Transmute-generated Stone .py, decode the embedded
    ...
    with open(src, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if not stripped:
                continue
            if not in_data:
                # Legacy comment-based metadata (older Transmute versions
...
def _py_is_stone(src: Path) -> bool:
    ...
    magic comment, since current Transmute versions don't write any
    identifying header comments. Also recognizes legacy files that
    have the `# Generated by Transmute - Philosopher's Stone Mode`
    ...
    if "# Generated by Transmute - Philosopher's Stone Mode" in head:
        return True
...
def can_embed_into(ext: str) -> bool:
    ext = ext.lower()
    return ext in _EMBED or ext in (".mkv", ".py", ".flac", ".m4a")
    ...
    if src_ext == ".py" and _py_is_stone(src):
        try:
            ...
            raise RuntimeError(
                ...
                f"or generated by an incompatible Transmute version.")
        cancel.check()
        progress(0.55)
